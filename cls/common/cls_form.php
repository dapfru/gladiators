...
(($this->mode==1)&&($this->document->getElementsByTagName("fields")||$this->document->getElementsByTagName("header")))
		...
		else
		{
				...
				{	
					...
					if(strstr(mysql_error(),"MySQL server has gone away")) $er.= "—ервер перегружен, ѕопробуйте зайти снова чуть позже.";
...
//if($idn) exit;
						...
						//иначе файл не записываетс€ - не получает $idn при insert после других операций
