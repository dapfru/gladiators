...
if($cancel)
{
	runsql("update ft_agreements set UserID2=null,Login2=null where (UserID2='$auth->user' or UserID1='$auth->user') and Approved=0");
}

if($approve)
{
	runsql("update ft_agreements set Approved=1,Date=unix_timestamp() where UserID1='$auth->user' and UserID2>0 and Approved=0");	
	$q=select("select RecordID from ft_agreements where UserID1='$auth->user' and UserID2>0 and Approved=1");
...
//-----------------------------------------------проверка на турнир-----------------------------
$q=select("select t.TournamentID,t.Name from ft_tournament_members m join ft_tournaments t on m.TournamentID=t.TournamentID and t.StatusID<2 and m.StatusID=0 where m.UserID='$auth->user' limit 0,1");


if(count($auth->rst[Gladiators])==0) print icon('error','Чтобы участвовать в боях, Вам необходимо <a href=/xml/city/market.php>купить гладиатора</a> на рынке или <a href=/xml/city/tavern.php>нанять наемника</a>.');
elseif($q[0])
{
	print icon('green',"Вы участвуете в турнире: <a href=tournaments.php?id=$q[0]>$q[1]</a>!");
}
else
{
...
//вызываем соперника на бой-----------

if($step&&($act=="battles"||$act=="duels"||$act=="series"&&$act=="survival")&&$id)
{

	$q=select("select RecordID,UserID2,LimitGlad,LimitSkl from ft_agreements where RecordID='$id' and Approved=0");
	...
	{
		mysql_query("update ft_agreements set UserID2=null,Login2=null where UserID2='$auth->user' and RecordID<>$id and Approved=0");
		mysql_query("update ft_agreements set UserID2='$auth->user',Login2='$auth->fullnick' where RecordID=$id");
		mysql_query("delete from ft_agreements where Approved=0 and UserID1='$auth->user;'");
...
if($result["redirect"]) print "<script>document.location.href='$result[redirect];'</script>";
