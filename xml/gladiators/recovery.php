...
//записываем пересчитанную физу
		$gladiators[$k][Stamina]=$v[Stamina];

//если нет травмы, можем поднять физу
if($v[Stamina]>0)
{
...
while($q=mysql_fetch_array($res))
{
	...
	//if(!$level) $leftcontent.=icon('green','Для начала тренировок, необходимо нанять тренера');
	if($level)
	{
		$leftcontent="<b>Нанятые специалисты</b><br><br>".$leftcontent;
	}
...
if($v[Injury]>0) print "<td colspan=3 align=center>травмирован</td>";
else
{
...
print "<tr bgcolor=$color align=center><td colspan=2>Работоспособность</td><td colspan=4>";


if($auth->rst[Staff][4]) print "Массажист <input type=text id=\"Health\" name=\"Health\" class=clear style=\"text-align:center;width:20px;background-color:$color\" value=$health>%";
else print "<a href=/xml/residence/staff.php?id=4>нанять массажиста</a>";

print "</td><td colspan=3>";

if($auth->rst[Staff][8]) print "Жрец <input type=text id=\"Morale\" name=\"Morale\" class=clear style=\"text-align:center;width:20px;background-color:$color\" value=$morale>%";
else print "<a href=/xml/residence/staff.php?id=8>нанять жреца</a>";

print "</td><td colspan=3>";

if($auth->rst[Staff][2]) print "Лекарь <input type=text id=\"Injury\" name=\"Injury\" class=clear style=\"text-align:center;width:20px;background-color:$color\" value=$injury>%";
else print "<a href=/xml/residence/staff.php?id=2>нанять лекаря</a>";
...
if($auth->rst[Staff][2]||$auth->rst[Staff][4]||$auth->rst[Staff][8]) print "<br>".icon('mobile',"Чтобы мгновенно восстановить работоспособность специалистов, отправьте SMS с текстом <b>\"glad $auth->user\"</b> (без кавычек) на номер <b>7099</b>. Услуга доступна в России, Украине и Казахстане. <a href=sms.php>Подробнее об услуге</a>");
