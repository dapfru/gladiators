...
function set_params($str)
    {
      global $rowcolor,$message,$sysmessage,$text,$test,$user,$img_url,$dbname,$site_url,$er,$id,$lang,$auth,$games,$r,$_POST,$_GET,$_GLOBALS,$act,$insertid;
...
elseif(is_array($_GET)) $_POST=$_GET;
	...
	if(!strlen($$par_name)) $$par_name=$_GLOBALS[$par_name];
		...
		elseif(strstr($par_name,"[")) 
		{
			...
			if($ob=="message"&&!$$par_name) $$par_name=message($var);
		}
		else
		{
...
//print "$_POST['team'] $par_name";
			...
			if(strlen($r[$par_name])) $$par_name=$r[$par_name];
...
function runsql($sql)
    {
	global $mysqlerror,$_GLOBAL;

	if($sql)
	{

		if(strstr($sql,"\$")||strstr($sql,"@")) $sql=set_params($sql);
		$sql=str_replace(";","",$sql);
	...
	//print "<b>".$sql.";</b><br><br>";
		...
		if($mysqlerror&&!$_GLOBAL['ignore_errors']) 
		{
