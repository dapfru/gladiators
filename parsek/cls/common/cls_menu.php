...
* ***********************************************************************/
function cls_menu($xml_url,$type,$level)
    {
	...
	$xml_document = xml_contents($site_path."xml/".$site.$xml_url.".xml");
        ...
        if(!$xml_document) {print "$site_path"."xml/".$site.$xml_url.".xml not found"; return false;}
...
//print "error reading XML: $tag<br>";
	...
	$toolbaritems = $this->xml_document->getElementsByTagName("item");
        ...
        {
		$item= new cls_menuitems($toolbaritems[$i],$this);
		$ar[$i]="<a href=\"$item->name\"><b>$item->caption</b></a>";
        }
...
//print $level." $file2==$file1&&$file1 len: ".strlen($ar[$level])."&&$ar1[$level]==$ar[$level])||($a==$act&&$act&&(".$ar1[$level]."==".$ar[$level].")))<br>";
	...
	if($this->menu->headerclass) print " class=".$this->menu->headerclass." ";
	if($this->menu->headerbg) print " background=".$this->menu->headerbg." ";
	if($this->menu->hcolor) print " bgcolor=".$this->menu->hcolor." ";
...
//	if((strstr($PHP_SELF,$file1)&&$file1&&$act&&$act==$ar[1])||(strstr($PHP_SELF,$file1)&&$file1&&!$act)||(strlen($ar[$level])&&$ar1[$level]==$ar[$level])||($a==$act&&$act&&($ar1[$level]==$ar[$level]))) print "bgcolor=dddddd";
	...
	if($this->name!=$type) print "<a href=\"".$this->name."\" class=".$this->menu->headerclass.">";

	if($this->menu->level==1) print "<b>".$this->caption."</b>";
	else print $this->caption;
	print "</td>";
    }
}
?>