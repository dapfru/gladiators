...
//	alert(input_players);
	...
	for(var i = 0; i<zones.length; i++)
	{
		...
		{
		  tacticsData=zones[i].players[j].attack+":";
		  tacticsData+=zones[i].players[j].power+":";
		  tacticsData+=zones[i].players[j].block+":";
		  tacticsData+=zones[i].players[j].courage;

				input_players.value += ";"+zones[i].players[j].GladiatorID+":"+tacticsData;
			//	alert(zones[i].players[j].attack);
				basePlayersCount ++;
