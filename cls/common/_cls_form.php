...
function set_form_params($str,$i)
	...
	global $site_path,$search,$im_array,$r,$REMOTE_ADDR,$_FILES,$_POST,$_GET,$id,$secpass,$lang,$auth,$er;
...
function Draw()
	...
	if($this->select) $r=select(settags($this->select));
...
//print $this->name."($this->val==$r1[0]&&$this->val)||".$r[$this->name]."==$r1[0]||(($this->text==$r1[1]||$this->text==$n)&&!".$r[$this->name].")";
				print "<input  class=\"radio\" name=\"".$this->name."\" type=\"radio\"  value=\"".$r1[0]."\"";
		...
		case "image":
			...
			srand();
			if(strlen($r[$this->name])>0) print "<img src=\"$site_url"."cls/images/image.php?id=$r[0]&table=$form->table&dbname=$dbname&rnd=".mt_rand(0,1000)."\" border=0><br>";
