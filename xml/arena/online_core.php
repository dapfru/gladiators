...
{
	print icon("error","Бой не найден");
}
else
{
...
function ch()
{
	if(pause==0)
	{
		document.game.pause.value="Продолжить";
		pause=1;
	}
	else
	{
		document.game.pause.value="Пауза";
		pause=0;
	}
}
...
function m_line() {
	...
	document.game.start.value="Закончить";
...
//print $type."<br>";
	...
	elseif($type=="12") 
	{
		...
		if ($retreat==0)
			$c= "<div class=green></div></td><td>...$name2 увернулся";
		else
	...
	elseif($type=="13") 
	{
		...
		$c= "<div class=green></div></td><td>...но не дотянулся";
	}
	elseif($type=="14") 
	{
	...
	elseif($type=="23") 
	{
		$c= "$name пришел в себя";
	}
	elseif($type=="24") 
	{
		$c= "$name впал в ярость!";
	}
	elseif($type=="25") 
	{
	...
	elseif($type=="50") 
	{
		$c= "<div class=lightblue></div></td><td><font color=#00f6ff><b>$name бросил оружие и сдался!</font>";
	}
	elseif($type=="51") 
	{
		//$b="bgcolor=170140";
		$c= "<div class=lightblue></div></td><td><font color=#00f6ff><b>$name потерял сознание!</font>";
	}
	elseif($type=="52") 
	{
		$c= "<div class=lightblue></div></td><td><font color=#00f6ff><b>$name ранен в руку, выронил оружие и сдался!</font>";
	}
	elseif($type=="53") 
	{
		$id2=next($v);
		$name2=gladiatorname($id2);
		$c= "<div class=lightblue></div></td><td><font color=#00f6ff><b>$name2 успел прижать упавшего $name ногой и $name сдался!</font>";
	}
	elseif($type=="73") 
	{
		$id2=next($v);
		$name2=gladiatorname($id2);
		$c= "<font color=yellow><b>$name2 остался с $name сражаться один на один!</b></font>";
	}
	elseif($type=="91") 
	{
		...
		{
			if($draw)
				$c= "<b>Раунд завершился вничью, счет $score1-$score2</b>";
			else
				$c= "<b>Победитель раунда - $name, счет $score1-$score2</b>";
		}
		else
		{
			if($draw)
				$c= "<b>Раунд завершился вничью</b>";
			else
				$c= "<b>Победитель раунда - $name</b>";
		}
	}
	elseif($type=="92") 
	{
		...
		if($oneprev)
			$c= "<b>Против $name на арену выходит $name2</b>";
		else
			$c= "<b>На арену выходят $name и $name2</b>";
	}
	...
	elseif($type=="60") 
	{
		...
		if($counter==0)
			$c= "<font color=yellow><b>$name нанес подряд уже $value удара!</b></font>";
		elseif($counter=="1")
	...
	elseif($type=="62") 
	{
		...
		if($counter=="0")
			$c= "<font color=yellow><b>Противники поочередно обмениваются ударами, не уступая друг другу!</b></font>";
		elseif($counter=="1")
			$c= "<font color=yellow><b>Противники безжалостно рубят друг друга!</b></font>";
		elseif($counter=="2")
			$c= "<font color=yellow><b>Противники сражаются на равных, не уступая друг другу!</b></font>";
	}
	elseif($type=="64") 
	{
		...
		if($counter=="0")
			$c= "<font color=yellow><b>$name отбивается одновременно от $value-х противников!</b></font>";
		elseif($counter=="1")
...
$scorepoints1=reset($res[ScorePoints]);
$scorepoints2=next($res[ScorePoints]);

if($score1>$score2) $winner="Победил <b>".$rst1[Login]."!</b>";
elseif($score2>$score1) $winner="Победил <b>".$rst2[Login]."!</b>";
elseif($game[TournamentID]&&$scorepoints1!=$scorepoints2) $winner="По очкам победил <b>".$rst2[Login]."!</b> (очки: $rst2[Login] - $scorepoints1, $rst2[Login] - $scorepoints2)";
elseif($game[TournamentID]) $winner="При равном количестве очков зрители отдали победу <b>".$rst2[Login]."!</b>";
else $winner="Бой завершился вничью!";
...
foreach($res[Gladiators] as $k=>$v)
{
	...
	if($rst1[Gladiators][$k]||$rst2[Gladiators][$k]) 
	{
		...
		if($v[Injury]) $gstr.=", <img src=/images/icons/inj.gif width=11px height=11px title=Травма><b> $v[Injury]</b>";
		$gstr.=", ";
...
$str.= "'1','','<table class=t width=100%  $b><td $t><font color=yellow><b>Деньги за бой: $res[Money]</font></td></table>', ";
...
hint +=\"<tr bgcolor=#3B484C><td width=90px><b>Возраст: </td width=110px><td><nobr>$v[Age]</td></tr>\"
hint +=\"<tr bgcolor=#515E64><td><b>Мораль: </td><td><nobr>\"+getStripe($v[Morale],true,5)+\" $v[Morale]</td></tr>\";
hint +=\"<tr bgcolor=#3B484C><td><b>Выносливость: </td><td>\"+getStripe($v[Vit])+\" ".round($v[Vit])."</td></tr>\";
hint +=\"<tr bgcolor=#515E64><td><b>Ловкость: </td><td>\"+getStripe($v[Dex])+\" ".round($v[Dex])."</td></tr>\";
hint +=\"<tr bgcolor=#3B484C><td><b>Точность: </td><td>\"+getStripe($v[Acc])+\" ".round($v[Acc])."</td></tr>\";
hint +=\"<tr bgcolor=#515E64><td><b>Сила: </td><td>\"+getStripe($v[Str])+\" ".round($v[Str])."</td></tr>\";
hint +=\"</table>\";
...
<input type=button class=button  value="Начать просмотр" onclick="startshow();">
...
<input type=button name=start value="Начать" onclick="startshow();">
</td>
<td><font color=black><center>

 <input type=radio value=1 name=1 onclick='speed=3500'> Медленно
 <input type=radio value=2 name=1 checked onclick='speed=1000'> Нормально
 <input type=radio value=3 name=1 onclick='speed=500'> Быстро

</font>
</td><td align="right" width=100px><input type=button value="Пауза" name=pause onclick="ch();">
</td><td width=20px><input type=button value="Шаг" name=step onclick="ch2();">
