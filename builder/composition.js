...
function makePlayersAndZones()
{
	...
	for(var i=0; i<playersCount; i++)
	{
		...
		var hintHtml = "<table border=0 width=100% bgcolor=#E5CEA6 cellspacing=1 cellpadding=4>";
		hintHtml += "<tr bgcolor=#515E64><td colspan=2><img src=/images/gladiators/"+players[i].TypeID+"_1.png><b>"+players[i].name+" ["+Math.round(players[i].Level)+"] </td></tr>";

		hintHtml += "<tr bgcolor=#3B484C><td width=90px><b>Возраст: </td><td width=110px><nobr>"+players[i].Age+"</td></tr>";

		hintHtml += "<tr bgcolor=#515E64><td><b>Здоровье </td><td><nobr>"+getStripe(players[i].Stamina,true)+" "+Math.round(players[i].Stamina)+"%</td></tr>";
		hintHtml += "<tr bgcolor=#3B484C><td><b>Мораль: </td><td><nobr>"+getStripe(players[i].Morale,true,5)+" "+players[i].Morale+"</td></tr>";

		hintHtml += "<tr bgcolor=#515E64><td><b>"+getAbilityName('Vit')+": </td><td>"+getStripe(players[i].Vit)+" "+Round(players[i].Vit,1)+"</td></tr>";
		hintHtml += "<tr bgcolor=#3B484C><td><b>"+getAbilityName('Dex')+": </td><td>"+getStripe(players[i].Dex)+" "+Round(players[i].Dex,1)+"</td></tr>";
		hintHtml += "<tr bgcolor=#515E64><td><b>"+getAbilityName('Acc')+": </td><td>"+getStripe(players[i].Acc)+" "+Round(players[i].Acc,1)+"</td></tr>";
		hintHtml += "<tr bgcolor=#3B484C><td><b>"+getAbilityName('Str')+": </td><td>"+getStripe(players[i].Str)+" "+Round(players[i].Str,1)+"</td></tr>";
	...
	LoadZones();
			...
			var tacticsHtml="<table width=212px border=0 bgcolor=78746C cellspacing=1 cellpadding=2 style='height:23px'>";
		tacticsHtml+="<TR  style='height:23px' class=header><td colspan=3><b><center><div id='personaltactics'></div></td></tr>";
		tacticsHtml+="<TR bgcolor=#545E61 style='height:23px'>";
		tacticsHtml+="<TD align=right width=53px>Защита</TD>";
		tacticsHtml+="<TD width=100>"+createSliderHtml('attackTrB','123')+"</TD>";
		tacticsHtml+="<TD width=53px>Атака</TD>";
		tacticsHtml+="</TR>";
		...
		tacticsHtml+="<TR bgcolor=#3B484C style='height:23px'>";
		tacticsHtml+="<TD colspan=3><div style='margin-left:80px'>Храбрость</div>";
		tacticsHtml+="<div style='margin-top:5px;margin-left:56px'>"+createSliderHtml('courageTrB','123')+"</div></TD>";
