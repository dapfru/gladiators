...
foreach($zones as $k)
{
	...
	$order[Squad][$a][$b]=$players[$j];
	$plid=$players[$j];
	$j++;


	$order[Tasks][$r[0]]=mt_rand(1,10);
	$order[Attack][$r[0]]=mt_rand(30,70);
	$order[Power][$r[0]]=mt_rand(30,70);
	$order[Block][$r[0]]=mt_rand(30,70);
	$order[Courage][$r[0]]=100;
	$order[Courage][$plid]=100;//mt_rand(30,70);
...
if($ar[TypeID]==2&&!($l==5)) $er.="Неправильная позиция ";
	...
	if(round($ar[LimitSkl])&&round($skl)>round($ar[LimitSkl])) $er.="Превышен лимит уровня гладиаторов ($ar[LimitSkl])"." ";
